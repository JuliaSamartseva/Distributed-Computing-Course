  ...
  public static void calculate(String[] args, int dimension) {
    ...
    gridComm =
        MPI.COMM_WORLD.Create_cart(
            new int[] {gridSize, gridSize}, new boolean[] {false, false}, true);
    ...
    if (gridCoords[0] != 0) {
      ...
      RowComm.Sendrecv_replace(
          BlockA, 0, blockSize * blockSize, MPI.INT, nextProcess, 0, MPI.ANY_SOURCE, 0);
    }
    ...
    if (gridCoords[1] != 0) {
      ...
      ColComm.Sendrecv_replace(
          BlockB, 0, blockSize * blockSize, MPI.INT, nextProcess, 1, MPI.ANY_SOURCE, 1);
    }
