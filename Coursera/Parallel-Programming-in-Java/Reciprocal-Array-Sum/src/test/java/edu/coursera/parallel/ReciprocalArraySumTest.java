...
public class ReciprocalArraySumTest extends TestCase {
  ...
  /**
   * A reference implementation of seqArraysum, in case the one in the main source file is
  ...
  private double parTestHelper(final int N, final boolean useManyTaskVersion, final int ntasks) {
    ...
    final String errMsg =
        String.format(
            "Mismatch in result for N = %d, expected = %f, computed = %f, absolute " + "error = %f",
            N, correct, sum, err);
  ...
  /**
   * Test that the two-task parallel implementation properly computes the results for a
  ...
  public void testParSimpleTwoMillion() {
    ...
    final String errMsg =
        String.format(
            "It was expected that the two-task parallel implementation would run at "
  ...
  /**
   * Test that the two-task parallel implementation properly computes the results for a hundred
   * million-element array.
   */
  ...
  /**
   * Test that the many-task parallel implementation properly computes the results for a
   * million-element array.
   */
  public void testParManyTaskTwoMillion() {
    ...
    final String errMsg =
        String.format(
            "It was expected that the many-task parallel implementation would run at "
                + "least %fx faster, but it only achieved %fx speedup",
            minimalExpectedSpeedup, speedup);
    assertTrue(errMsg, speedup >= minimalExpectedSpeedup);
  }
  ...
  /**
   * Test that the many-task parallel implementation properly computes the results for a hundred
   * million-element array.
   */
